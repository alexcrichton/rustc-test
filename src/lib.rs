...
pub enum TestFn {
    ...
    DynBenchFn(Box<FnMut(&mut Bencher) + Send>),
}
...
pub fn run_tests_console(opts: &TestOpts, tests: Vec<TestDescAndFn> ) -> io::Result<bool> {
    ...
    {
        let it = tests.iter().enumerate().map(|(i, t)| (len_if_padded(t), i));
        if let Some((_, i)) = it.max() {
            let t = &tests[i];
...
pub fn convert_benchmarks_to_tests(tests: Vec<TestDescAndFn>) -> Vec<TestDescAndFn> {
    ...
    tests.into_iter().map(|x| {
        let testfn = match x.testfn {
            DynBenchFn(mut bench) => {
                DynTestFn(Box::new(move || bench::run_once(|b| bench(b))))
            }
...
pub fn run_test(opts: &TestOpts,
    ...
    fn run_test_inner(desc: TestDesc,
        ...
        thread::spawn(move || {
            ...
            let result_guard = cfg.spawn(move || {
                if !nocapture {
                    __stdio::__SINK.with(|sink| {
                        *sink.borrow_mut() = Some(Box::new(Sink(data2)));
                    });
                    // TODO: override panic here as well?
                }
                testfn()
            }).unwrap();
    ...
    match testfn {
        DynBenchFn(mut bencher) => {
            let bs = ::bench::benchmark(|harness| bencher(harness));
...
impl Bencher {
    ...
    pub fn iter<T, F>(&mut self, mut inner: F) where F: FnMut() -> T {
        ...
        self.dur = time::precise_time_ns() - start;
    }
    ...
    pub fn auto_bench<F>(&mut self, mut f: F) -> stats::Summary where F: FnMut(&mut Bencher) {
        ...
        loop {
            ...
            let loop_run = time::precise_time_ns() - start;
